...
function calculatePriceAndTime(language, mimetype, count) {
  ...
  function getDeadLine(t) {
    const date = new Date("2022-06-16T10:30:00");
  ...
  const time = parseFloat(calculateTime(count, language, mimetype).toFixed(1));
  ...
  const deadline_date =
    ...
    " " +
    d.slice(2).join(":");
  let result = { price, time, deadline_date };
  return result;
}

console.log(calculatePriceAndTime("ua", "pdf", 1000));

module.exports = { calculatePriceAndTime };
